...
//!
//! Taken from https://github.com/embassy-rs/embassy/pull/5723/changes#diff-3f233e0aabfdd701efc0b7989f0a7ef784a01877894841191bf40fd47f6ac4d2
...
impl<'d, D: Driver<'d>> MscClass<'d, D> {
    ...
    pub async fn run<B: AsyncBlockDevice>(
        &mut self,
        block_device: &mut B,
        block_buf: &mut [u8],
    ) -> !
        ...
        B::Error: core::fmt::Debug,
    {
        loop {
            ...
            self.write_csw(cbw.tag, result.residue, result.status)
                .await?;
        }
    }
    ...
    async fn read_cbw(&mut self) -> Result<Option<Cbw>, EndpointError> {
        ...
        if n != raw.len() {
            log::warn!("msc: invalid CBW size {}", n);
            return Ok(None);
        }
        ...
        let cbw = match parse_cbw(&raw) {
            Ok(cbw) => cbw,
            Err(_) => {
                log::warn!("msc: invalid CBW payload");
                return Ok(None);
            }
        };
    ...
    async fn mode_sense_6<B: AsyncBlockDevice>(
        ...
        let mut data = [0u8; 4];
        data[0] = 3;
        data[2] = if block_device.is_write_protected() {
            0x80
        } else {
            0x00
        };
        ...
        B::Error: core::fmt::Debug,
    {
        ...
        for i in 0..blocks {
            if block_device
                ...
                .is_err()
            {
                log::warn!("msc: read_block failed");
                self.set_sense(
        ...
        B::Error: core::fmt::Debug,
    {
        ...
        if block_size == 0 || block_buf.len() < block_size {
            ...
            self.discard_out_data(cbw.data_transfer_length, block_buf)
                .await?;
            return Ok(CommandResult::failed(cbw.data_transfer_length));
        }
        ...
        for i in 0..blocks {
            ...
            if block_device
                ...
                .is_err()
            {
                log::warn!("msc: write_block failed");
        ...
        if block_device.flush().await.is_err() {
            log::warn!("msc: flush after write failed");
            self.set_sense(SENSE_KEY_MEDIUM_ERROR, ASC_WRITE_ERROR, ASCQ_NONE);
            return Ok(CommandResult::failed(0));
        }
